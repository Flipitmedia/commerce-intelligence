...
@router.get("/{store_id}/debug/sync")
def debug_sync(store_id: str, token: str = ""):
    """Debug: hace sync de Shopify paso a paso y retorna cada detalle."""
    import httpx
    from app.database import query, get_db
    from app.sync_shopify import (
        get_shopify_token, period_to_date_range, shopify_fetch_all,
        extract_order_data, SHOPIFY_API_VERSION,
    )
    ...
    steps.append({
        "step": "1_credentials",
        "domain": store["shopify_domain"],
        "client_id_preview": store["shopify_client_id"][:8] + "..." if store["shopify_client_id"] else "(empty)",
        "has_secret": bool(store["shopify_client_secret"]),
    })

    # Step 2: Token
    try:
        tok = get_shopify_token(store)
        steps.append({"step": "2_token", "ok": True, "preview": f"{tok[:8]}...{tok[-4:]}", "is_shpat": tok.startswith("shpat_")})
    except Exception as e:
        steps.append({"step": "2_token", "ok": False, "error": str(e)})
        return {"steps": steps}

    # Step 3: Date range
    since, until = period_to_date_range(periodo)
    steps.append({"step": "3_date_range", "periodo": periodo, "since": since, "until": until})

    # Step 4: Direct API test
    try:
        with httpx.Client(timeout=30) as c:
            url = f"https://{store['shopify_domain']}/admin/api/{SHOPIFY_API_VERSION}/orders.json"
            r = c.get(url, params={"status": "any", "created_at_min": since, "created_at_max": until, "limit": 3},
                      headers={"X-Shopify-Access-Token": tok})
            data = r.json()
            orders_test = data.get("orders", [])
            steps.append({
                "step": "4_direct_api_test",
                "http_status": r.status_code,
                "orders_found": len(orders_test),
                "sample": [{"id": o["id"], "created_at": o.get("created_at"), "total_price": o.get("total_price")} for o in orders_test[:3]],
            })
    except Exception as e:
        steps.append({"step": "4_direct_api_test", "error": str(e)})

    # Step 5: shopify_fetch_all (created_at)
    try:
        orders_created = shopify_fetch_all(store["shopify_domain"], tok, "orders.json", {
            "status": "any", "created_at_min": since, "created_at_max": until, "limit": 250,
        })
        steps.append({"step": "5_fetch_created", "count": len(orders_created)})
    except Exception as e:
        steps.append({"step": "5_fetch_created", "error": str(e)})
        return {"steps": steps}

    # Step 6: shopify_fetch_all (updated_at)
    try:
        orders_updated = shopify_fetch_all(store["shopify_domain"], tok, "orders.json", {
            "status": "any", "updated_at_min": since, "updated_at_max": until, "limit": 250,
        })
        steps.append({"step": "6_fetch_updated", "count": len(orders_updated)})
    except Exception as e:
        steps.append({"step": "6_fetch_updated", "error": str(e)})
        return {"steps": steps}
    ...
    for o in orders_created + orders_updated:
        if o["id"] not in seen_ids:
            seen_ids.add(o["id"])
            orders.append(o)
    steps.append({"step": "7_merge", "total": len(orders)})

    # Step 8: Extract and filter
    order_rows = []
    periodos_found = {}
    for order in orders:
        od, lines, refunds = extract_order_data(order, store_id, store_tz)
        periodos_found[od["periodo"]] = periodos_found.get(od["periodo"], 0) + 1
        if od["periodo"] == periodo:
            order_rows.append(od)
    steps.append({
        "step": "8_extract_filter",
        "matched_periodo": len(order_rows),
        "periodos_found": periodos_found,
        "sample_extracted": [{"id": r["id"], "periodo": r["periodo"], "total_price": r["total_price"], "current_total_price": r["current_total_price"]} for r in order_rows[:3]],
    })

    # Step 9: DB state BEFORE
    before = query("SELECT COUNT(*) as c FROM orders WHERE store_id = ? AND periodo = ?", (store_id, periodo))
    count_before = before[0]["c"] if before else 0
    steps.append({"step": "9_db_before", "orders_in_db": count_before})

    # Step 10: Write to DB (only if we have orders)
    if order_rows:
        try:
            with get_db() as conn:
                ...
                for row in order_rows:
                    ...
                    conn.execute(f"INSERT INTO orders ({cols}) VALUES ({placeholders})", tuple(row.values()))
            steps.append({"step": "10_db_write", "ok": True, "rows_written": len(order_rows)})
        except Exception as e:
            steps.append({"step": "10_db_write", "ok": False, "error": str(e)})
    else:
        steps.append({"step": "10_db_write", "skipped": True, "reason": "0 orders matched"})

    # Step 11: DB state AFTER
    after = query("SELECT COUNT(*) as c FROM orders WHERE store_id = ? AND periodo = ?", (store_id, periodo))
    count_after = after[0]["c"] if after else 0
    sample_after = query("""
        SELECT id, periodo, total_price, current_total_price, financial_status
        FROM orders WHERE store_id = ? AND periodo = ?
        LIMIT 5
    """, (store_id, periodo))
    steps.append({
        "step": "11_db_after",
        "orders_in_db": count_after,
        "sample": [dict(r) for r in sample_after] if sample_after else [],
    })

    return {"store_id": store_id, "periodo": periodo, "steps": steps}
